$(document).ready(function() {
    ...
    function loadMyClasses() {
        $.ajax({
            url: 'http://127.0.0.1:5015/my_classes',
            method: 'GET',
                    ...
                    data.forEach(cls => {
                        const card = $(`
                            <div class="class-card">
                                ...
                                <p>Người đăng ký: ${cls.registered_by ? `<a href="#" class="user-link" data-username="${cls.registered_by}">${cls.registered_by}</a>` : 'Chưa có người đăng ký'}</p>
    ...
    function loadRegisteredClasses() {
        $.ajax({
            url: 'http://127.0.0.1:5015/registered_classes',
            method: 'GET',
            data: { username: username },
            dataType: 'json',
            success: function(data) {
    ...
    $(document).on('click', '.user-link', function(event) {
        ...
        $.ajax({
            url: 'http://127.0.0.1:5015/get_user_info',
            method: 'GET',
            data: { username: username },
            dataType: 'json',
            success: function(data) {
                if (data.error) {
                    ...
                    userInfoContainer.html(`
                        ...
                        <button class="accept-class btn btn-primary" data-class-id="${cls.id}">Nhận</button>
                    `);
    ...
    $(document).on('click', '.accept-class', function(event) {
        ...
        $.ajax({
            url: 'http://127.0.0.1:5015/accept_registration',
            method: 'POST',
